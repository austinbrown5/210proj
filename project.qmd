...
```
```{r Graph3}
player_data %>% filter(NBA__3ptapg > 0.5) %>% filter(NCAA_fgapg > 0.3) %>%
  filter(NBA_g_played > 20) %>% filter(NCAA_games > 20) %>%
ggplot(mapping = aes(x = NCAA_fgpct, y = NBA__3ptpct)) +
  geom_point() +
  geom_smooth(method = "lm") + 
  labs(title = "College FG Percentage vs. NBA 3pt. Percentage", 
       x = "NCAA FG %", y = "NBA 3pt. %") + ylim(0, 0.5) + xlim(.2, .75)
```
...
player_data <- player_data %>%
  filter(NBA__3ptapg > 0.5) %>% filter(NCAA__3ptapg > 0.5) %>%
  filter(NBA_g_played > 20) %>% filter(NCAA_games > 20) %>% 
  filter(NCAA_ftapg > 0.3) 
...
#nba3pt <- lm(NBA__3ptpct ~ NCAA_ft + NCAA__3ptpct + NCAA_fgpct +  
#               NCAA_ft*position, data = model_data)
nba3pt <- lm(NBA__3ptpct ~ NCAA__3ptpct, data = model_data)
tidy(nba3pt)
